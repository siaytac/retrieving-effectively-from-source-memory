testsourcememory <- function(Nf, memorytraces, testsource, c, gsys, n, bestmatch) {
  ...
  for(counttrace in 1:n){ #go through each source
    ...
    trace = matrix(trace[(1+Nf*(counttrace)):(Nf*(counttrace+1))], ncol=1) #best matching trace source
